  ...
  rtcSignalingWSHandler = createAutoReconnectWS(url, {
    ...
    onmessage: (event) => {
      ...
      if (data.type === "voice_member_update" && data.members) {
        // รองรับ room_id ด้วย (สำหรับอนาคต)
...
function updateVoiceMembersUI(members, roomId = null) {
  // ตรวจสอบว่าอัปเดตเฉพาะห้องปัจจุบันเท่านั้น (ถ้ามีหลายห้องหรือ sidebar)
  if (roomId && String(roomId) !== String(window.currentChatRoomId)) return;
