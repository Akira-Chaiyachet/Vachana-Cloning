...
window.currentWsRoomId = null; // [ADD] เก็บ ID ของห้องที่ WebSocket เชื่อมต่ออยู่
...
function connectWebSocket(roomId) {
  // 1. [REVISED] ตรวจสอบว่ากำลังเชื่อมต่อกับห้องเดิมที่ active อยู่แล้วหรือไม่
  if (
    window.socket &&
    window.socket.readyState === WebSocket.OPEN &&
    String(window.currentWsRoomId) === String(roomId)
  ) {
    console.log(`WS: Already connected to room ${roomId}. Skipping.`);
    return;
  }

  // 2. [REVISED] ถ้ามี socket เก่าอยู่ (สำหรับห้องอื่น หรือปิดไปแล้ว) ให้ปิดการเชื่อมต่อเก่าก่อน
  if (window.socket) {
    console.log(
      `🔌 Closing old WebSocket connection (for room ${window.currentWsRoomId})...`
    );
    window.socket.onclose = null; // ป้องกันไม่ให้ onclose ของเก่าพยายาม reconnect
    window.socket.close();
    window.socket = null;
  }
  ...
  window.currentWsRoomId = roomId; // [ADD] บันทึก ID ห้องของ socket ปัจจุบัน
