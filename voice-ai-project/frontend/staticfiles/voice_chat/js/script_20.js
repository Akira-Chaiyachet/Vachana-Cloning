        ...
        .then(data => {
            if (!data.isAuthenticated) {
                if (inviteCodeFromUrl) {
                    sessionStorage.setItem("pending_invite", inviteCodeFromUrl);
                }
            ...
            if (inviteCodeFromUrl) {
                searchRoom_link(inviteCodeFromUrl);
...
function joinRoomPopup() {
    ...
    fetch("/api/check-auth/")
        ...
        .then(data => {
            ...
            // เรียก API เข้าห้องเมื่อผู้ใช้ล็อกอินแล้ว
            ...
            .then(response => {
                if (response.headers.get("Content-Type").includes("text/html")) {
                    return response.text(); // รับข้อมูลเป็น HTML แทน JSON
                }
                return response.json(); // ถ้าเป็น JSON
            })
            .then(data => {
                if (typeof data === "string") {
                    console.log("📢 Response HTML:", data); // แสดง HTML ใน console
...
function searchRoom_link(inviteCode) {
    fetch(`/api/search-room/${inviteCode}/`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`ไม่สามารถค้นหาห้องได้ สถานะ: ${response.status}`);
            }
            return response.json();
        })
        .then(data => {
            if (data.error) {
                ...
                if (currentUrlParams.has("invite")) {
                    currentUrlParams.delete("invite");
                    const newPath = window.location.pathname;
                    const newSearchString = currentUrlParams.toString();
                    const newRelativeUrl = newSearchString ? `${newPath}?${newSearchString}` : newPath;
                    window.history.replaceState({}, document.title, newRelativeUrl);
                }
            }
        })
        .catch(error => {
            console.error("เกิดข้อผิดพลาดในการค้นหาห้อง (searchRoom_link):", error);
