document.addEventListener("DOMContentLoaded", function () {
  ...
  if (roomId && isVoiceJoined(roomId)) {
    setTimeout(() => joinVoiceRoom(), 500); // หรือ joinVoiceRoom(roomId) ถ้า function รองรับรับ param
  } else {
    // ไม่ join อะไรทั้งสิ้น socket จะเชื่อมเมื่อ user join เท่านั้น!
    // (ไม่ต้อง connectRTCSignalingSocket(roomId) ตอนโหลดเว็บ)
  }
...
function safeLoadRoom(roomId, roomName, inviteCode) {
  // กรณี user ยังอยู่ใน voice
  if (window.rtcIsJoined) {
    if (confirm("คุณกำลังพูดคุยในห้องเสียงอยู่ ต้องการออกจากห้องเดิมและเข้าห้องใหม่ใช่หรือไม่?")) {
      loadRoom(roomId, roomName, inviteCode); // cleanup และ join ใหม่
    } else {
      // ไม่ทำอะไร
      return;
    }
  } else {
    // ไม่ได้ join voice เดิม → เปลี่ยนห้องปกติ
