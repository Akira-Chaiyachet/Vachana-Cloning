...
let _voiceLoad = { el: null, timer: null, firstPlayed: false };
...
function showVoiceLoading(tip = 'กำลังเชื่อมต่อเสียง…') {
  ...
  el.innerHTML = `
    ...
    <div style="font-size:12px;opacity:.8">
      ถ้ายังไม่ได้ยิน ให้คลิกปุ่มด้านบนเพื่ออนุญาตเสียง (autoplay)
    </div>
  ...
  const unlockBtn = el.querySelector('#voice-loading-unlock');
...
function updateVoiceLoadingText(txt) {
  try {
    const span = _voiceLoad.el?.querySelector('#voice-loading-text');
...
function hideVoiceLoading() {
  ...
  if (_voiceLoad.el) { _voiceLoad.el.remove(); _voiceLoad.el = null; }
  _voiceLoad.firstPlayed = false;
}

async function unlockAudioPlayback() {
  // 1) resume AudioContext (บางหน้าใช้ WebAudio อาจถูก suspend ไว้)
  try {
    // ถ้ามี TTS player อยู่
    const ac = window.voicePipeline?.player?.ac;
    if (ac && ac.state !== 'running') await ac.resume();
  } catch { }
  // 2) สั่ง play() กับทุก audio remote
  const audios = getRoomAudios();
  for (const a of audios) {
    try { await a.play(); } catch { }
  }
  updateVoiceLoadingText('กำลังเริ่มเสียง…');
}
