...
// --- VVV เพิ่ม Event Listener เพื่อแจ้งเตือน Server ก่อนปิดหน้าต่าง VVV ---
window.addEventListener('beforeunload', function (e) {
  // Event นี้จะทำงานเมื่อผู้ใช้กำลังจะปิดแท็บ หรือเปลี่ยนหน้าเว็บ
  if (socket && socket.readyState === WebSocket.OPEN) {
    // เราจะส่ง action 'leaving' ไปยัง server
    // หมายเหตุ: วิธีนี้ไม่รับประกันว่าจะทำงานได้ 100% เพราะเบราว์เซอร์อาจปิดการเชื่อมต่อก่อนที่ข้อความจะถูกส่งสำเร็จ
    // การจัดการที่ฝั่ง server เมื่อ 'disconnect' ยังคงเป็นวิธีที่ดีที่สุด
    // คุณจะต้องไปเขียนโค้ดที่ Server เพื่อจัดการกับ action 'leaving' นี้ด้วย
