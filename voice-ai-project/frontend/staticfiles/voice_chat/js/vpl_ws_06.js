...
(function () {
  ...
  class VoiceGatewayClient {
    ...
    _emit(msg) { const fn = this.handlers[msg.type]; if (fn) fn(msg); }
    ...
    sendAudioFrame(seq, pcm16) {
      ...
      this._send({ type: 'audio_chunk', seq, format: 'pcm_s16le', sampleRate: 16000, durationMs: 20 });
      this.ws.send(pcm16.buffer);
    }
    setTargetLang(v) { this._send({ type: 'set_target_lang', value: v }); }
    setSourceLang(v) { this._send({ type: 'set_src_lang', value: v }); }
    setSpeakerUrl(url) { this._send({ type: 'set_speaker_url', value: url }); }
  }

  // ========================= TTSPlayer (WebAudio) =========================
  class TTSPlayer {
    constructor(opts) {
      this.opts = Object.assign({ crossfadeMs: 20, targetGain: 1.0 }, opts || {});
    ...
    _flush() {
      ...
      this._playing = true;
      try { this.onSpeak && this.onSpeak(); } catch { }

      let t = Math.max(this.ac.currentTime + 0.02, this._lastEnd || this.ac.currentTime + 0.02);
    ...
    clear() { this.queue.length = 0; this._lastEnd = null; }
    stop() { this.clear(); /* ไม่ปิด this.ac เพื่อกันเงียบ */ }
  }
  function hardDuckOriginalAudio(active) {
    // ปรับ selector ให้ตรงกับ audio element ของคุณ (เช่น class="rtc-remote")
    const audios = document.querySelectorAll('audio.rtc-remote, audio[id^="rtc-remote-"], audio[id^="voice-audio-"]');
    audios.forEach((a) => {
      if (active) {
  ...
  document.addEventListener('click', async () => {
    try { await window.voicePipeline?.player?.ac?.resume(); } catch { }
  }, { once: true });
  ...
  // สร้าง pipeline
  window.voicePipeline = new VoicePipeline({
    url: (location.protocol === 'https:' ? 'wss://' : 'ws://') + location.host + '/ws/voice',
    userId: String(window.myRTCUserId || ''),
    getRoomId: () => String(window.currentChatRoomId || ''),
    ...
    onMetrics: (m) => {
      ...
      el.textContent = `Latency — STT: ${ms.stt || 0}ms | MT: ${ms.mt || 0}ms | TTS: ${ms.tts || 0}ms | E2E: ${ms.e2e || 0}ms${hint}`;
    },
    log: (...args) => console.log('[VPL]', ...args)
  });
  ...
  // ✅ รอให้ WS ตอบ hello_ok ก่อน
    ...
    try { window.voicePipeline.ws.setSpeakerUrl(SPEAKER_URL); } catch {}
