(function () {
  ...
  class VoiceGatewayClient {
    constructor({ url, userId, getUserId, getRoomId, log }) {
      this.url = url;
      // รองรับทั้งส่ง userId เป็น string หรือฟังก์ชัน
      this.getUserId =
    ...
    connect() {
      ...
      this.ws.onopen = () => {
        // ทำ sessionId ให้ปลอดภัยทุกเบราว์เซอร์
        let sid = String(Date.now());
        try {
          if (self?.crypto?.randomUUID) sid = self.crypto.randomUUID();
        } catch {}

        const sendHelloWhenReady = (deadline = Date.now() + 10000) => {
          ...
          if (Date.now() < deadline) {
            // รอให้ template/Django ใส่ค่า userId/roomId เสร็จก่อน
            setTimeout(() => sendHelloWhenReady(deadline), 120);
          } else {
            // ยังไม่มี uid → ปิดเองด้วยเหตุผลชัดเจนกันค้าง
  ...
  window.voicePipeline = new VoicePipeline({
    ...
    userId: () => String(window.myRTCUserId || ""), // ✅ ใช้เป็น function จะปลอดภัยกว่า
