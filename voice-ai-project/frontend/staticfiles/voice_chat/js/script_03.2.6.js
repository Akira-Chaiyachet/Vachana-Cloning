...
function connectWebSocket(roomId) {
    ...
    socket.onmessage = function (event) {
        ...
        switch (data.type) {
            case "user_join":
                console.log(`👋 สมาชิกใหม่เข้าห้อง: ${data.user?.display_name || 'Unknown'}`);
                loadRoomMembers(); // โหลดรายชื่อสมาชิกใหม่ทั้งหมด
                break;
            case "user_leave":
                console.log(`👋 สมาชิกออกจากห้อง: ${data.user?.display_name || 'Unknown'}`);
                loadRoomMembers(); // โหลดรายชื่อสมาชิกใหม่ทั้งหมด
                break;
        ...
        .then(data => {
            ...
            if (Array.isArray(data.members) && data.members.length > 0) {
                data.members.forEach(member => {
                    ...
                    console.log(`[DEBUG] Processing member: ${member.username}, Role: '${member.role}'`);
                    ...
                    const roleClass = member.role ? `role-${member.role.toLowerCase()}` : ''; // <--- 🐞 BUG FIX 1: แปลงเป็นตัวพิมพ์เล็กเสมอ
                    li.innerHTML = `
                        <img src="${member.profile_image || '/media/default/profile.jpg'}" 
                            onerror="this.src='/media/default/profile.jpg'" 
                            class="profile-img">
                        <span class="user-display-name">${escapeHTML(displayName)}</span><span class="crown-icon ${roleClass}"></span>
