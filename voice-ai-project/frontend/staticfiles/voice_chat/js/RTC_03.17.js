  ...
  rtcSignalingWSHandler = createAutoReconnectWS(url, {
    ...
    onmessage: (event) => {
      ...
      if (data.type === "voice_member_update" && data.members) {
        updateVoiceMembersUI(data.members); // อัปเดตรายชื่อทันที
      }
      // TODO: RTC peer logic...
    },
...
function updateVoiceMembersUI(members) {
    ...
    // แสดงจำนวนสมาชิก voice room (ถ้าต้องการ)
    const count = members ? members.length : 0;
    const countLabel = `<div class="voice-member-count">${count === 0
        ? "ยังไม่มีใครอยู่ในห้องเสียง"
        : `${count} คนในห้องเสียง`
      }</div>`;
...
function handleJoinVoiceClick() {
  if (rtcIsJoined && rtcActiveRoomId !== currentChatRoomId) {
    ...
    const oldRoomName = localStorage.getItem("currentRoomName") || rtcActiveRoomId;
    const newRoomName = document.getElementById("roomTitle").textContent || currentChatRoomId;
    if (confirm(`คุณต้องการออกจากห้องเสียง "${oldRoomName}" และเข้าร่วมห้อง "${newRoomName}" หรือไม่?`)) {
      leaveVoiceRoom(rtcActiveRoomId);
