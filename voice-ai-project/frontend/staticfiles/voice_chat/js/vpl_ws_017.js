(function () {
  ...
  class VoiceGatewayClient {
    constructor({ url, userId, getUserId, getRoomId, log }) {
      this.url = typeof url === "function" ? url() : url; // กันส่ง function object
      this.getUserId =
        typeof userId === "function"
          ...
          ? getUserId
          : () => String(userId || "");
      this.getRoomId = typeof getRoomId === "function" ? getRoomId : () => "";
      this.log = log || (() => {});
      this.ws = null;
      this.handlers = {};
    ...
    connect() {
      ...
      this.ws.onopen = () => {
        ...
        try {
          if (self?.crypto?.randomUUID) sid = self.crypto.randomUUID();
        } catch {}
        const tryHello = (deadline = Date.now() + 10000) => {
          const uid = String(this.getUserId() || "");
          const rid = String(this.getRoomId() || "");
          if (uid && rid) {
            this._send({
              type: "hello",
              sessionId: sid,
          ...
          if (Date.now() < deadline)
            return setTimeout(() => tryHello(deadline), 120);
          this.log("WS hello not sent: userId/roomId empty", { uid, rid });
          try {
            this.ws?.close(1008, "missing uid/rid");
          } catch {}
        };
        this.log("WS open → wait uid/rid …");
        tryHello();
      };
    ...
    setSpeakerUrl(u) {
      this._send({ type: "set_speaker_url", value: u });
    }
  }
  ...
  const resolveGatewayUrl = () =>
    window.__VOICE_GATEWAY_URL ||
    (location.protocol === "https:" ? "wss://" : "ws://") +
      location.host +
      "/ws/voice";

  window.voicePipeline = new VoicePipeline({
    url: resolveGatewayUrl(), // ✅ ต้องเรียก
    userId: () => String(window.myRTCUserId || ""), // ✅ อ่านสดทุกครั้ง
  ...
  // apply langs + mic after hello_ok (ปลอดภัยสุด)
  window.voicePipeline.ws.on("hello_ok", async () => {
    try {
      window.voicePipeline.setSpeakerUrl("/static/song/xtts/01.wav");
    } catch {}
    try {
