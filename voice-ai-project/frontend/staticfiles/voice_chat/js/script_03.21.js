document.addEventListener("DOMContentLoaded", function () {
  // --- ป้องกัน auto-connect WebSocket ถ้า user อยู่โหมดล่องหน ---
  fetch("/users/api/profile/me/")
    .then((res) => res.json())
    .then((data) => {
      if (data && data.user && data.user.status === "invisible") {
        // ถ้าเป็น invisible, set flag ป้องกัน auto-connect (ใช้ร่วมกับ script_03.2.8.js)
        localStorage.setItem("preventAutoConnect", "1");
      } else {
        localStorage.removeItem("preventAutoConnect");
      }
    });
  ...
  if (appContainer && toggleButton && sidebar) {
    ...
    console.error(
      "Sidebar toggle elements not found. The toggle feature will be disabled."
    );
  }
  ...
  if (roomId && isVoiceJoined(roomId)) {
    setTimeout(() => joinVoice(true), 500); // auto join, delay เล็กน้อยเพื่อให้ socket พร้อม
  }
  const urlParams = new URLSearchParams(window.location.search);
  const inviteCodeFromUrl = urlParams.get("invite");
  const pendingInviteCodeFromSession = sessionStorage.getItem(
    "pending_invite_code_after_auth"
  );
  ...
  fetch("/api/check-auth/")
    ...
    .then((data) => {
      if (!data.isAuthenticated) {
        ...
        if (codeToRemember) {
          ...
          console.log(
            "Homepage: Unauthenticated, invite code found. Storing and redirecting to login. Code:",
            codeToRemember
          );

          let redirectToLoginNextParam = "/";
          if (inviteCodeFromUrl) {
            redirectToLoginNextParam =
              window.location.pathname + window.location.search;
          } else {
            redirectToLoginNextParam = `/join-room/${codeToRemember}/`;
          }
          window.location.href = `/users/login/?next=${encodeURIComponent(
            redirectToLoginNextParam
          )}`;
          return;
        }
        ...
        if (finalInviteCode) {
          ...
          console.log(
            "Homepage (Authenticated): No invite code to process. Loading last room from localStorage."
          );
    ...
    .catch((error) => {
      console.error(
        "เกิดข้อผิดพลาดในการตรวจสอบสถานะล็อกอิน หรือประมวลผล invite code:",
        error
      );
    });
  if (roomId && isVoiceJoined(roomId)) {
    ...
    if (!window.joinedVoiceRoom) {
      setTimeout(joinVoice, 600); // delay เล็กน้อยให้ ws/rtc พร้อม
    }
  }
  ...
  console.log("DOM ready. My user id:", window.myRTCUserId, "roomId:", roomId);

  // โหลดรายชื่อสมาชิกห้อง
  if (roomId) fetchAndDisplayRoomMembers(roomId);

  // เช็คว่าต้อง auto join voice (ถ้าเคย join ไว้)
  if (isVoiceJoined(roomId)) {
    console.log("auto join voice on reload");
    joinVoice(true);
  } else {
    // ถ้าไม่ join voice แต่ต้องการ sync รายชื่อ voice ด้วย, connect socket, ขอ voice_update
...
document.addEventListener("click", function (event) {
  ...
  const currentUserDisplayTrigger = document.querySelector(
    ".current-user-display"
  ); // Trigger ของ currentUserMenu
  if (currentUserMenu && currentUserDisplayTrigger) {
    // ตรวจสอบว่า elements มีอยู่จริง
    if (
      currentUserMenu.style.display === "block" &&
      !currentUserMenu.contains(event.target) && // คลิกไม่ได้อยู่ข้างใน currentUserMenu
      !currentUserDisplayTrigger.contains(event.target)
    ) {
      // และคลิกไม่ได้อยู่ที่ .current-user-display
...
function submitCreateRoomFormFromModal() {
  // ชื่อใหม่ (หรือจะใช้ชื่อเดิมก็ได้ถ้าไม่สับสน)
    ...
    .then((data) => {
      ...
      data.rooms.forEach((room) => {
        ...
        li.innerHTML = `
            ...
            `;
        li.onclick = () =>
          loadRoom(room.id, escapeHTML(room.name), escapeHTML(inviteCode)); // ใช้ค่าที่ผ่านการตรวจสอบแล้ว
...
function updateRoomInSidebar(roomId, newName, newImageUrl) {
  console.log("DEBUG: updateRoomInSidebar called with:", {
    roomId,
    newName,
    newImageUrl,
  });
  ...
  if (roomListItem) {
    ...
    if (inviteCode) {
      roomListItem.setAttribute(
        "onclick",
        `loadRoom('${roomId}', '${escapeHTML(newName)}', '${escapeHTML(
          inviteCode
        )}')`
      );
    }
    ...
    console.log(
      "DEBUG: currentRoomId in localStorage:",
      currentRoomIdInLocalStorage
    );
    console.log(
      "DEBUG: Comparison (currentRoomIdInLocalStorage === String(roomId)):",
      currentRoomIdInLocalStorage === String(roomId)
    );

    if (roomTitleElement && currentRoomIdInLocalStorage === String(roomId)) {
      // currentRoomId จาก localStorage เป็น string
...
function updateUserInUserList(userId, newDisplayName, newProfileImageUrl) {
  console.log(
    "DEBUG: updateUserInUserList called for user:",
    userId,
    "with new name:",
    newDisplayName,
    "and image:",
  ...
  if (userListItem) {
    ...
    if (userNameSpan) {
      ...
      console.log(
        `DEBUG: User ID ${userId} name updated in userList to: ${newDisplayName}`
      );
    }
    if (userImageImg && newProfileImageUrl) {
      ...
      console.log(
        `DEBUG: User ID ${userId} image updated in userList to: ${newProfileImageUrl}`
      );
    }
  } else {
...
function loadRoom(roomId, roomName, inviteCode) {
  ...
  loadRoomMembers();
  fetchAndDisplayRoomMembers(roomId);
  connectRTCSignalingSocket(roomId);

  // >>> เริ่มการเชื่อมต่อ Voice Chat <<<
  if (window.startVoiceChat) {
    console.log(`🎤 Attempting to start voice chat for room ${roomId}`);
    window.startVoiceChat(roomId);
  }

  if (typeof currentRoomId !== "undefined") currentRoomId = roomId;
}
...
function joinRoomPopup() {
  ...
  const roomName =
    ...
    `ห้อง ${inviteCode}`;
  const roomImage =
    document.getElementById("modalRoomImage")?.src || "/media/default/room.jpg";
...
function addRoomToSidebar(
  ...
  let displayName = isNew
    ? `${escapeHTML(roomName)} <b style="color: red;">(ห้องใหม่!)</b>`
    : escapeHTML(roomName);
...
function searchRoom_link(inviteCode) {
  ...
  fetch(`/api/search-room/${inviteCode}/`)
    .then((response) => {
      if (!response.ok) {
        // เพิ่มการตรวจสอบนี้เพื่อความปลอดภัย
    ...
    .then((data) => {
      ...
      } else if (data.room_name && data.room_image !== undefined) {
        // ตรวจสอบข้อมูลก่อนเรียก
        showPopupModal(data.room_name, data.room_image, inviteCode);
      } else {
    ...
    .catch((error) => {
      console.error(
        "เกิดข้อผิดพลาดในการค้นหาห้อง (searchRoomOrProcessInvite):",
        error
      );
...
function showPopup(roomName, roomImage, inviteCode) {
  ...
  document.getElementById("popupRoomImage").src =
    roomImage || "/media/default/room.jpg";
