  ...
  class TTSPlayer {
    ...
    constructor(opts) {
      this.opts = Object.assign(
        {
          targetGain: 1.0,
          baseCrossfadeMs: 20, // crossfade สูงสุด ระดับชุด
          minPlayUnitMs: 260, // ถ้าเป็นหลายชิ้น จะรวมถึงจุดนี้ก่อนค่อยปล่อย
          jitterStartMs: 80, // กันจึ้กตอนชิ้นแรก (รอ buffer เล็กน้อย)
          boundaryXfadeMs: 8, // crossfade เล็กๆ ระหว่างชิ้นในชุด
          startLookaheadMs: 40, // เวลานำสำหรับ scheduler
          onDucking: null,
          log: () => {},
        },
        opts || {}
    ...
    async pushChunk(arrayBuffer, meta = {}) {
      ...
      if (token !== this.currentToken) return; // late chunk → ทิ้ง

      const buf = await this._decode(arrayBuffer);
      if (!buf) return;

      // กันชิ้นสั้นมาก (เสียงสั้น <120ms มักโดน fade กินจน “เหมือนเงียบ”)
      const minChunk = 0.12; // 120ms
      const chunk =
        buf.duration < minChunk ? this._padSilence(buf, minChunk) : buf;

      this.pending.push(chunk);
      this.pendingDur += chunk.duration;

      // ถ้าใช้ “ก้อนเดียว” ฝั่งเซิร์ฟเวอร์จะตามด้วย tts_end → เราจะ flush ตอน endStream
      // แต่ถ้าบัฟเฟอร์ถึงเกณฑ์ ก็ปล่อยได้เลย (ช่วยลดดีเลย์)
      if (this.pendingDur * 1000 >= this.opts.minPlayUnitMs) {
    ...
    async endStream(token) {
      ...
      // end ของคิวเก่า → ไม่ทำอะไร
      if (token !== this.currentToken) return;

      // ถ้ามีของค้างอยู่ ให้ปล่อยทั้งหมดเป็นชุดสุดท้ายทันที
    ...
    _flushPlayUnit(isFinal) {
      ...
      const baseFade = Math.min(
        this.opts.baseCrossfadeMs / 1000,
        durSum * 0.35
      );
      gGroup.gain.linearRampToValueAtTime(
        this.opts.targetGain,
        t0 + Math.max(0.005, baseFade)
      );
      ...
      const boundaryFade = Math.max(
        0.003,
        Math.min(this.opts.boundaryXfadeMs / 1000, durSum * 0.1)
      );
      ...
      for (let i = 0; i < this.pending.length; i++) {
        ...
        if (i < this.pending.length - 1) {
          const endHold = Math.max(
            localStart + smallIn,
            localEnd - boundaryFade
          );
    ...
    _padSilence(buf, targetSec) {
      const outFrames = Math.max(
        buf.length,
        Math.floor(targetSec * buf.sampleRate)
      );
      const out = this.ac.createBuffer(
    ...
    clear() {
      // ล้างทุกอย่างแบบไม่ต้องรอ fade (ใช้เวลาศูนย์)
      this._hardFlush(0);
    }
  }
  ...
  class VoicePipeline {
    constructor({
      ...
      this.player = new TTSPlayer({
        onDucking: (active) => {
          onDucking && onDucking(active); // ให้ UI duck เสียงอื่น (ถ้ามี)
          this.rec.setMuted(!!active); // << ปิด/เปิดไมค์ผู้ใช้ขณะ TTS พูด
        },
        log: (...a) => this.log(...a),
      });
      this.targetLang = "off";
      ...
      this.ws.on("tts_chunk", async (h) => {
        // header จะมาก่อน แล้วตามด้วย binary (h.bytes) → ผู้เล่นจัดการต่อคิวเอง
