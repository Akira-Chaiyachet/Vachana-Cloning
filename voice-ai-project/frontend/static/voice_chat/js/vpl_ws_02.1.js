(function () {
  ...
  class TTSPlayer {
    ...
    async beginStream(token) {
      ...
      // เราไม่ flush เมื่อมี token ใหม่แล้ว
      this.currentToken = this.currentToken || token || "no-token";
      this._endedForToken.delete(token || this.currentToken);
      this._armJitterStart();
    }

    async pushChunk(arrayBuffer) {
