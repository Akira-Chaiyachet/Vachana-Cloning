  ...
  fetch("/create-room/", {
    ...
    .then((data) => {
      if (data.error) {
        ...
        closeGenericModal();
        // ไม่จำเป็นต้องเรียก loadRooms() อีก เพราะ addRoomToSidebar ได้เพิ่มห้องใหม่ใน UI แล้ว
      }
    })
    .catch((error) => {
...
// ตัวแปรสำหรับเก็บ ID ของผู้ใช้ปัจจุบันชั่วคราว เพื่อใช้เป็น Fallback
let currentUserIdForUpdate = null;
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (userData && userData.user) {
        setTimeout(() => {
          currentUserIdForUpdate = userData.user.id; // เก็บ ID ผู้ใช้ไว้
        ...
        if (data.user) {
          ...
          if (userAvatarImgElement && updatedUser.profile_image_url) {
            ...
            updateUserInUserList(
              // ใช้ ID จาก response ถ้ามี, ถ้าไม่มี (เป็น undefined) ให้ใช้ ID ที่เก็บไว้เป็น fallback
              updatedUser.id || currentUserIdForUpdate,
              updatedUser.display_name,
              updatedUser.profile_image_url
            ); // อัปเดตใน userList ด้วย
          }
...
function submitChangeUsernamePassword() {
  ...
  const form = document.getElementById(
    "genericModalChangeUsernamePasswordForm"
  );
  const errorMessagesDiv = document.getElementById(
    "modalChangeUsernamePasswordErrorMessages"
  );
  ...
  if (!currentPassword) {
    ...
    // ถ้ามีการพยายามเปลี่ยน password
    if (newPassword1 !== newPassword2) {
      validationError = "Password ใหม่และการยืนยัน Password ไม่ตรงกัน";
    } else if (newPassword1 && newPassword1.length < 8) {
      // ตรวจสอบความยาวถ้ามีการกรอก password ใหม่
      validationError = "Password ใหม่ต้องมีอย่างน้อย 8 ตัวอักษร";
    }
  } else if (!newUsername) {
    // ถ้าไม่ได้กรอก password ใหม่ และไม่ได้กรอก username ใหม่ด้วย
...
function processActualUsernamePasswordChange() {
  ...
  fetch("/users/api/change-credentials/", {
    ...
    .then((response) => {
      ...
      return response.text().then((text) => {
        // ถ้าไม่ใช่ JSON ให้โยน error พร้อมข้อความดิบ
        throw new Error(
          `Server response was not JSON. Status: ${
            response.status
    ...
    .then((res) => {
      // res = { ok, status, data }
      if (!res.ok) {
        // ถ้า HTTP Status ไม่ใช่ 2xx (เช่น 400, 500)
        // data อาจจะมี error object จาก server
        const serverError =
          (res.data && (res.data.error || res.data.detail)) ||
          `เกิดข้อผิดพลาดจาก Server (สถานะ: ${res.status})`;
      ...
      alert(
        res.data.message ||
          "Username/Password อัปเดตสำเร็จ! กรุณา Login ใหม่อีกครั้ง"
      );
    ...
    .catch((error) => {
      console.error(
        "เกิดข้อผิดพลาดขณะเปลี่ยน Username/Password (catch block):",
        error
      );
      closeGenericModal(); // ปิด Popup ยืนยัน (ถ้ายังเปิดอยู่)
      ...
      setTimeout(() => {
        // หน่วงเวลาเล็กน้อยเพื่อให้ DOM ของฟอร์มพร้อม
                ...
                for (const field in parsedErrors) {
                  if (Array.isArray(parsedErrors[field])) {
                    parsedErrors[field].forEach((msg) => {
                      errorHtml += `<li>${
                        field === "__all__" ? "" : escapeHTML(field) + ": "
                      }${escapeHTML(msg)}</li>`;
                    });
                  } else {
                    errorHtml += `<li>${
                      field === "__all__" ? "" : escapeHTML(field) + ": "
                    }${escapeHTML(parsedErrors[field])}</li>`;
                  }
                }
                errorHtml += "</ul>";
                errorDiv.innerHTML = errorHtml;
              } else {
                // ถ้า parse ได้ แต่ไม่ใช่ object ที่เราคาดหวัง
                errorDiv.textContent = escapeHTML(error.message);
              }
            } catch (e) {
              // ถ้า error.message ไม่ใช่ JSON string
              errorDiv.textContent = escapeHTML(error.message);
            }
          } else {
...
// ฟังก์ชันสำหรับเปิด Modal แก้ไขโปรไฟล์ห้อง (ปรับปรุงให้ดึงข้อมูลจาก Backend โดยตรง)
async function displayEditRoomProfileModal() {
  ...
  if (settingsMenu && settingsMenu.style.display === "block") {
    ...
    const trigger = document.querySelector(
      '[data-menu-trigger-for="settingsMenu"]'
    );
    if (trigger && typeof resetMenuIcon === "function") {
  ...
  try {
    // ดึงข้อมูลห้องล่าสุดจาก Backend
    const response = await fetch(`/rooms/api/room-details/${currentRoomId}/`);
    if (!response.ok) {
      throw new Error(`ไม่สามารถดึงข้อมูลห้องได้: ${response.statusText}`);
    }
    const roomData = await response.json();

    const roomName = roomData.name;
    const roomImage = roomData.image_url;

    openGenericModal({
      title: `แก้ไขโปรไฟล์ห้อง: ${escapeHTML(roomName)}`,
      bodyHTML: formHtml,
      footerHTML: `
                <button class="modal-button-default" onclick="closeGenericModal()">ยกเลิก</button>
    ...
    setTimeout(() => {
      ...
      const roomCurrentImageElem = document.getElementById(
        "modalEditRoomCurrentImage"
      );
      ...
      if (roomCurrentImageElem) {
        roomCurrentImageElem.src = roomImage;
      }
    }, 0); // ใช้ setTimeout เพื่อให้แน่ใจว่า DOM elements ถูก render แล้ว
  } catch (error) {
    console.error("เกิดข้อผิดพลาดขณะโหลดข้อมูลห้องเพื่อแก้ไข:", error);
    alert("ไม่สามารถโหลดข้อมูลห้องเพื่อแก้ไขได้: " + error.message);
  }
}
...
function submitEditRoomProfileForm() {
  ...
  fetch(`/rooms/api/update-room-profile/${currentRoomId}/`, {
    // API endpoint สำหรับอัปเดตโปรไฟล์ห้อง
    method: "POST", // หรือ PUT/PATCH ตามการออกแบบ API ของคุณ
    body: formData,
    headers: { "X-CSRFToken": getCSRFToken() },
  })
          ...
          if (typeof data.error === "object" && data.error !== null) {
            ...
            for (const field in data.error) {
              if (Array.isArray(data.error[field])) {
                ...
                errorHtml += `<li>${
                  field === "__all__" ? "" : escapeHTML(field) + ": "
                }${escapeHTML(data.error[field])}</li>`;
              }
            }
            errorHtml += "</ul>";
            errorMessagesDiv.innerHTML = errorHtml;
    ...
    .catch((error) => {
      console.error("เกิดข้อผิดพลาดขณะอัปเดตโปรไฟล์ห้อง:", error); // Log error for debugging
      const displayError = error.responsePayload
        ? JSON.stringify(error.responsePayload)
        : error.message || "เกิดข้อผิดพลาดบางอย่าง กรุณาลองใหม่";
      alert(displayError);
    });
}
