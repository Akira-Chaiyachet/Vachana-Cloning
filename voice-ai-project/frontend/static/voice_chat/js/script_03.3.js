    ...
    fetch("/get-rooms/")
        ...
        .then(data => {
            ...
            data.rooms.forEach(room => {
                ...
                li.dataset.inviteCode = room.invite_code; // เพิ่ม data-invite-code
                li.innerHTML = `
                <img src="${room.image_url}" onerror="this.src='/media/default/room.jpg'">
                <span>${room.name}</span>
            `;
                li.onclick = () => loadRoom(room.id, escapeHTML(room.name), escapeHTML(room.invite_code)); // ใช้ escapeHTML
